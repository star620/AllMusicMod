...
namespace AllMusicMod.UIKit
{
	...
	internal class UIView
	{
		...
		// UIKit 用 Main.UIScaleMatrix 绘制（UI 缩放坐标系），命中判定必须用同系坐标：
		// Main.MouseScreen 已经把鼠标映射到 UI 坐标系上，避免 UI scale != 100% 时错位。
		protected static int MouseX { get { return (int)Main.MouseScreen.X; } }
		protected static int MouseY { get { return (int)Main.MouseScreen.Y; } }
